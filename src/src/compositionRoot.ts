...
async function initializeService(config: ServiceConfig): Promise<void> {
  ...
  try {
    logger.info(`ðŸ”§ Initializing ${config.name}...`)
    ...
    logger.info(`âœ… ${config.name} initialized successfully (${duration.toFixed(2)}ms)`)
    ...
    if (config.critical) {
      logger.error(`âŒ CRITICAL: ${errorMsg}`)
      throw new Error(errorMsg)
    } else {
      logger.warn(`âš ï¸ NON-CRITICAL: ${errorMsg}`)
...
export async function initServices(): Promise<void> {
  ...
  if (servicesInitialized) {
    logger.info('âš ï¸ Services already initialized')
  ...
  initializationPromise = (async () => {
    ...
    try {
      logger.info('ðŸš€ Initializing Protozoa domain services...')
      ...
      logger.info(`ðŸŽ‰ All services initialized successfully in ${totalDuration.toFixed(2)}ms`)
      
    } catch (error) {
      perfLogger.endTimer(totalTimer)
      logger.error(`âŒ Service initialization failed: ${error}`)
      throw error
    } finally {
...
export async function disposeServices(): Promise<void> {
  if (!servicesInitialized) {
    logger.info('âš ï¸ Services not initialized, nothing to dispose')
    return
  }

  logger.info('ðŸ§¹ Disposing all services...')
  ...
  for (const [name, service] of services) {
    try {
      if (service && typeof service.dispose === 'function') {
        ...
        logger.info(`ðŸ—‘ï¸ Disposed ${name}`)
      }
    } catch (error) {
      logger.warn(`âš ï¸ Failed to dispose ${name}: ${error}`)
    }
  }
  ...
  logger.info(`âœ… All services disposed in ${disposeDuration.toFixed(2)}ms`)
}
...
export function getService<T>(serviceName: string): T | null {
  ...
  if (!service) {
    logger.warn(`âš ï¸ Service '${serviceName}' not found in registry`)
    return null
  }
  return service as T
}
...
export async function healthCheck(): Promise<Record<string, boolean>> {
  ...
  for (const [name, service] of serviceRegistry.entries()) {
    try {
      ...
      logger.error(`âŒ Health check failed for ${name}: ${error}`)
      health[name] = false
    }
  }
...
export async function restartService(serviceName: string): Promise<boolean> {
  try {
    ...
    if (!config) {
      logger.error(`âŒ Service config not found: ${serviceName}`)
      return false
    }
    ...
    logger.info(`âœ… Service restarted successfully: ${serviceName}`)
    return true
    
  } catch (error) {
    logger.error(`âŒ Failed to restart service ${serviceName}: ${error}`)
    return false
  }
}
