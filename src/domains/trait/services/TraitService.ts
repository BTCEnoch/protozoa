...
export class TraitService implements ITraitService {
  ...
  /**
   * Mutate a single trait value (helper method for trait inheritance)
   * @param traitKey - The trait identifier
   * @param value - The current trait value
   * @returns Mutated trait value
   */
  public mutateTrait(traitKey: string, value: any): any {
    // Apply simple mutation logic - can be extended for more complex mutations
    if (typeof value === 'number') {
      const range = this.#traitRanges.get(traitKey)
      if (range) {
    ...
    return value // No mutation for unknown types
  }
  ...
  #generateVisualTraits(blockNumber: number): VisualTraits {
    ...
    return {
      ...
      shape:
        this.#availableShapes[((hash[4] ?? 0) || 0) % this.#availableShapes.length] || 'circle',
  ...
  #generateColor(hash: number[], offset: number): string {
    const r = (hash[offset % hash.length] || 128) % 256
    const g = (hash[(offset + 1) % hash.length] || 128) % 256
    const b = (hash[(offset + 2) % hash.length] || 128) % 256
  ...
  #generateTraitValue(traitName: string, hash: number[], offset: number): number {
    ...
    const normalizedValue = ((hash[offset % hash.length] || 500) % 1000) / 1000
  ...
  #mutateVisualTraits(visual: VisualTraits, rate: number, mutations: MutationRecord[]): void {
    ...
    if (Math.random() < rate) {
      ...
      mutations.push(
        this.#createMutationRecord('visual', 'opacity', oldOpacity, visual.opacity, rate)
      )
    }
  }
  ...
  #mutateBehavioralTraits(
    ...
    if (Math.random() < rate) {
      ...
      mutations.push(
        this.#createMutationRecord('behavioral', 'speed', oldSpeed, behavioral.speed, rate)
      )
    }
  }
  ...
  #updateCacheHitRate(hit: boolean): void {
    ...
    if (hit) {
      this.#metrics.cacheHitRate =
        (this.#metrics.cacheHitRate * (totalOperations - 1) + 100) / totalOperations
    } else {
      this.#metrics.cacheHitRate =
        (this.#metrics.cacheHitRate * (totalOperations - 1)) / totalOperations
    }
  }
}
