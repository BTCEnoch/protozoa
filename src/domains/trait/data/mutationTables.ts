...
export const MutationTableHelpers = {
  ...
  calculateMutationIntensity(blockInfo: BlockInfo): number {
    ...
    logger.debug(
      `Blockchain difficulty ${difficulty} maps to ${intensityLevel.description || 'unknown'}`
    )
   ...
   */
  mutateNumericTrait(traitName: string, currentValue: number, intensity: number): number {
    ...
    const algorithm =
      NUMERIC_MUTATION_ALGORITHMS[mapping.algorithm as keyof typeof NUMERIC_MUTATION_ALGORITHMS]
    if (!algorithm) {
    ...
    logger.debug(
      `Mutated ${traitName}: ${currentValue} → ${mutatedValue} (${algorithm?.name || 'unknown'})`
    )
    return mutatedValue
  },
  ...
  calculateMutationDrift(mutationHistory: MutationRecord[], traitName: string): number {
    ...
    for (let i = 1; i < traitMutations.length; i++) {
      ...
      if (currentMutation && previousMutation) {
        const current =
          typeof currentMutation.newValue === 'number' ? (currentMutation.newValue as number) : 0
        const previous =
          typeof previousMutation.newValue === 'number' ? (previousMutation.newValue as number) : 0
