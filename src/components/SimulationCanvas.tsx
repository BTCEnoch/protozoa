...
export function SimulationCanvas() {
  ...
  useEffect(() => {
    ...
    if (isInitializedRef.current) {
      logger.warn(
        '🤔 [STRICTMODE] Simulation already initialized, skipping duplicate initialization'
      )
      return
    }
    ...
    async function initializeSimulation() {
      try {
        ...
        try {
          ...
          logger.warn('🤔 Particle system already exists, cleaning and reusing...')
          ...
          if (particleSystem.particles.length > 0) {
            ...
            logger.info(`🗑️ Cleaned ${existingIds.length} existing particles from system`)
          }
        }
        ...
        if (currentParticleCount === 0) {
          ...
          if (finalCount !== successfulSpawns) {
            logger.warn(
              `🤔 Particle count mismatch! Expected: ${successfulSpawns}, Actual: ${finalCount}`
            )
          }
        } else {
        ...
        logger.info('🔍 Camera setup complete', {
          position: {
            x: renderingService.camera.position.x,
            y: renderingService.camera.position.y,
            z: renderingService.camera.position.z,
          },
          target: { x: 0, y: 0, z: 0 },
        ...
        logger.info('🔍 Scene status', {
          ...
          canvasSize: {
            width: renderingService.renderer.domElement.width,
            height: renderingService.renderer.domElement.height,
          },
    ...
    return () => {
      ...
      if (isInitializedRef.current) {
        logger.info('🗑️ Cleaning up simulation...')
        ...
        try {
          ...
          if (mainSystem) {
            ...
            if (particleIds.length > 0) {
              ...
              logger.info(`🗑️ Removed ${particleIds.length} particles from system`)
            }
            // Remove the system from the map
            systems.delete('main-organisms')
            logger.debug('🗑️ Particle system cleaned up')
          }

          // Dispose services
          particleService.dispose()
          logger.info('🗑️ Services disposed successfully')
    ...
    <div style={{ width: '100%', height: '100%', position: 'relative', background: '#000' }}>
      ...
      {isLoading && (
        ...
        >
          ...
          />
          <div style={{ fontSize: '14px', marginBottom: '8px' }}>INITIALIZING BITCOIN ORDINALS</div>
