...
export class RNGService implements IRNGService {
  ...
  #config: RNGConfig = { 
    ...
    maxChainLength: 1000 // New: prevent infinite rehash chains
  }
  
  #state: RNGState = { 
    seed: 12345, 
    counter: 0,
    chainLength: 0 // New: track rehash chain length
  }
  ...
  #metrics: RNGMetrics = { 
    ...
    source: "manual",
    rehashCount: 0 // New: track rehash operations
  }
  ...
  getPurposeRng(purpose: string): () => number {
    if (!this.#purposeRngs.has(purpose)) {
      ...
      logger.debug("🎯 Purpose RNG created", { purpose, seed: purposeSeed })
    }
