...
// Import logger for standalone functions
const logger = createServiceLogger("TraitService");
...
export class TraitService implements ITraitService {
  ...
  #generateVisualTraits(blockNumber: number): VisualTraits {
    ...
    return {
      ...
      shape: this.#availableShapes[((hash[4] ?? 0) || 0) % this.#availableShapes.length] || 'circle',
  ...
  #generateTraitValue(traitName: string, hash: number[], offset: number): number {
    ...
    // Ensure we're working with numeric ranges only
    if (typeof range.min !== 'number' || typeof range.max !== 'number') {
      logger.warn(`Non-numeric range for trait ${traitName}, returning default`)
      return 1.0;
    }
  ...
  #mutateNumericTrait(currentValue: number, traitName: string, variance: number): number {
    ...
    // Ensure we're working with numeric ranges only
    if (typeof range.min !== 'number' || typeof range.max !== 'number') {
      logger.warn(`Non-numeric range for trait ${traitName}, skipping mutation`)
      return currentValue;
    }
