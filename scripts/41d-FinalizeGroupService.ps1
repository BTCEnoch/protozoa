# 41d-FinalizeGroupService.ps1 - Finalize GroupService Implementation
# Completes GroupService with utility methods, dependency injection, and export
...
try {
    Write-InfoLog "Finalizing GroupService implementation"

    $groupServicePath = Join-Path $ProjectRoot "src/domains/group/services/GroupService.ts"
    
    # Add the final methods and class completion
    $groupServiceFinal = @'
...
'@
    ...
    Write-ErrorLog "GroupService finalization failed: $($_.Exception.Message)"
    exit 1
} 